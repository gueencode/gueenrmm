...
If (!(test-path $env:programdata\gueenrmm\temp\)) {
    New-Item -ItemType Directory -Force -Path $env:programdata\gueenrmm\temp\
}

If (!(test-path $env:programdata\gueenrmm\temp\curpsxpolicy.txt)) {
    $curexpolicy = Get-ExecutionPolicy

    (
        Write-Output $curexpolicy
    )>$env:programdata\gueenrmm\temp\curpsxpolicy.txt
}
...
$curpsxpol = Get-Content -Path $env:programdata\gueenrmm\temp\curpsxpolicy.txt;
