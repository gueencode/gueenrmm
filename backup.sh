...
SCRIPT_VERSION="16"
SCRIPT_URL='https://raw.githubusercontent.com/gueencode/gueenrmm/master/backup.sh'
...
POSTGRES_USER=$(grep -w USER /rmm/api/gueenrmm/gueenrmm/local_settings.py | sed 's/^.*: //' | sed 's/.//' | sed -r 's/.{2}$//')
POSTGRES_PW=$(grep -w PASSWORD /rmm/api/gueenrmm/gueenrmm/local_settings.py | sed 's/^.*: //' | sed 's/.//' | sed -r 's/.{2}$//')
...
sudo -u postgres psql -d gueenrmm -c "vacuum full logs_auditlog"
sudo -u postgres psql -d gueenrmm -c "vacuum full logs_pendingaction"
...
pg_dump --dbname=postgresql://"${POSTGRES_USER}":"${POSTGRES_PW}"@127.0.0.1:5432/gueenrmm | gzip -9 > ${tmp_dir}/postgres/db-${dt_now}.psql.gz
...
cat /rmm/api/gueenrmm/gueenrmm/private/log/django_debug.log | gzip -9 > ${tmp_dir}/rmm/debug.log.gz
cp /rmm/api/gueenrmm/gueenrmm/local_settings.py /rmm/api/gueenrmm/app.ini ${tmp_dir}/rmm/
cp /rmm/web/.env ${tmp_dir}/rmm/env
cp /rmm/api/gueenrmm/gueenrmm/private/exe/mesh*.exe ${tmp_dir}/rmm/
