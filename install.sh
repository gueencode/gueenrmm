...
SCRIPT_VERSION="57"
SCRIPT_URL='https://raw.githubusercontent.com/gueencode/gueenrmm/master/install.sh'
...
sudo -u postgres psql -c "CREATE DATABASE gueenrmm"
...
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE gueenrmm TO ${pgusername}"

sudo mkdir /rmm
sudo chown ${USER}:${USER} /rmm
sudo mkdir -p /var/log/celery
sudo chown ${USER}:${USER} /var/log/celery
git clone https://github.com/gueencode/gueenrmm.git /rmm/
cd /rmm
...
NATS_SERVER_VER=$(grep "^NATS_SERVER_VER" /rmm/api/gueenrmm/gueenrmm/settings.py | awk -F'[= "]' '{print $5}')
...
MESH_VER=$(grep "^MESH_VER" /rmm/api/gueenrmm/gueenrmm/settings.py | awk -F'[= "]' '{print $5}')
...
echo "${localvars}" > /rmm/api/gueenrmm/gueenrmm/local_settings.py
...
SETUPTOOLS_VER=$(grep "^SETUPTOOLS_VER" /rmm/api/gueenrmm/gueenrmm/settings.py | awk -F'[= "]' '{print $5}')
WHEEL_VER=$(grep "^WHEEL_VER" /rmm/api/gueenrmm/gueenrmm/settings.py | awk -F'[= "]' '{print $5}')
...
pip install --no-cache-dir -r /rmm/api/gueenrmm/requirements.txt
...
rmmservice="$(cat << EOF
[Unit]
Description=gueenrmm uwsgi daemon
After=network.target postgresql.service

[Service]
User=${USER}
Group=www-data
WorkingDirectory=/rmm/api/gueenrmm
...
WorkingDirectory=/rmm/api/gueenrmm
Environment="PATH=/rmm/api/env/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
ExecStart=/rmm/api/env/bin/daphne -u /rmm/daphne.sock gueenrmm.asgi:application
Restart=always
RestartSec=3s
...
upstream gueenrmm {
    server unix:////rmm/api/gueenrmm/gueenrmm.sock;
}
...
server {
    ...
    access_log /rmm/api/gueenrmm/gueenrmm/private/log/access.log combined if=\$ignore_ua;
    error_log /rmm/api/gueenrmm/gueenrmm/private/log/error.log;
    ssl_certificate ${CERT_PUB_KEY};
    ssl_certificate_key ${CERT_PRIV_KEY};
    ssl_ciphers 'ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305:DHE-RSA-AES128-GCM-SHA256:DHE-RSA-AES256-GCM-SHA384';

    location /static/ {
        root /rmm/api/gueenrmm;
    }

    location /private/ {
        internal;
        add_header "Access-Control-Allow-Origin" "https://${frontenddomain}";
        alias /rmm/api/gueenrmm/gueenrmm/private/;
    }
...
CELERYD_PID_FILE="/rmm/api/gueenrmm/%n.pid"
CELERYD_LOG_FILE="/var/log/celery/%n%I.log"
CELERYD_LOG_LEVEL="ERROR"

CELERYBEAT_PID_FILE="/rmm/api/gueenrmm/beat.pid"
...
echo "${meshtoken}" | tee --append /rmm/api/gueenrmm/gueenrmm/local_settings.py > /dev/null
...
node node_modules/meshcentral/meshctrl.js --url wss://${meshdomain}:443 --loginuser ${meshusername} --loginpass ${MESHPASSWD} AddDeviceGroup --name gueenrmm
sleep 5
MESHEXE=$(node node_modules/meshcentral/meshctrl.js --url wss://${meshdomain}:443 --loginuser ${meshusername} --loginpass ${MESHPASSWD} GenerateInviteLink --group gueenrmm --hours 8)
...
sed -i 's/ADMIN_ENABLED = True/ADMIN_ENABLED = False/g' /rmm/api/gueenrmm/gueenrmm/local_settings.py
