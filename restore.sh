...
SCRIPT_VERSION="32"
SCRIPT_URL='https://raw.githubusercontent.com/gueencode/gueenrmm/master/restore.sh'
...
tmp_dir=$(mktemp -d -t gueenrmm-XXXXXXXXXXXXXXXXXXXXX)
...
cp $tmp_dir/rmm/local_settings.py /rmm/api/gueenrmm/gueenrmm/
cp $tmp_dir/rmm/env /rmm/web/.env
gzip -d $tmp_dir/rmm/debug.log.gz
cp $tmp_dir/rmm/django_debug.log /rmm/api/gueenrmm/gueenrmm/private/log/
cp $tmp_dir/rmm/mesh*.exe /rmm/api/gueenrmm/gueenrmm/private/exe/
...
pgusername=$(grep -w USER /rmm/api/gueenrmm/gueenrmm/local_settings.py | sed 's/^.*: //' | sed 's/.//' | sed -r 's/.{2}$//')
pgpw=$(grep -w PASSWORD /rmm/api/gueenrmm/gueenrmm/local_settings.py | sed 's/^.*: //' | sed 's/.//' | sed -r 's/.{2}$//')

sudo -u postgres psql -c "DROP DATABASE IF EXISTS gueenrmm"
...
PGPASSWORD=${pgpw} psql -h localhost -U ${pgusername} -d gueenrmm -f $tmp_dir/postgres/db*.psql
