...
# remove non domain joined device from grmm and unjoin them from Azure Ad
...
if ($domain.toupper().contains('DOMAIN')) {
    ...
    $ChkReg = Test-Path 'HKLM:\SOFTWARE\gueenrmm\'
    If ($ChkReg -eq $True) {
        $regrmm = Get-ItemProperty -Path HKLM:\SOFTWARE\gueenrmm\        
