...
- A real (internet resolvable) domain is needed to generate a Let's Encrypt wildcard cert. _If you cannot afford to purchase a domain ($12 a year) then you can get one for free at [freenom.com](https://www.freenom.com/)_
    - example.local is __NOT__ a real domain. No you [don't have to expose your server](faq.md#can-i-run-gueen-rmm-locally-behind-nat-without-exposing-anything-to-the-internet) to the internet
- A TOTP based authenticator app. Some popular ones are Google Authenticator, Authy and Microsoft Authenticator.

#### Update Recommendations

!!!note
    We highly recommend staying current with updates (at least every 3 months when you update your SSL certs is a good minimum) while Gueen RMM is still working towards its 1.0 release.<br/><br/>
    Until we reach production release, there may be architectural changes that may be made to Gueen RMM and only a regular patching schedule is supported by developers.
...
1. Get the public IP of your server with `curl https://icanhazip.gueenrmm.io`
...
```bash
wget https://raw.githubusercontent.com/gueencode/gueenrmm/master/install.sh
chmod +x install.sh
./install.sh
```
...
Make sure your local DNS server (or agents hosts file) have your Gueen RMM server IP addresses for the 3 domain names: `rmm`, `api` and `mesh`

### Agents exist outside the private network/subnet - Setup Port Forwarding

If you have agents outside your local network: Make sure the public DNS servers have A records for the 3 Gueen RMM server domain names: `rmm`, `api` and `mesh`

Login to your router/NAT device.

1. Set your grmm server as a static IP (Using a DHCP reservation is usually safer)
2. Create 2 port forwarding rules. `TCP Port 443` and `TCP Port 4222` to your grmm servers private IP address.
...
2. Agents need to be able to connect to your server via DNS lookup (hosts file, local DNS, smoke signals etc.).
    - Test from agent: `ping rmm.example.com`. Should result in the IP of your Gueen RMM server
    - Test from agent: `ping api.example.com`. Should result in the IP of your Gueen RMM server
    - Test from agent: `ping mesh.example.com`. Should result in the IP of your Gueen RMM server
