...
Gueen RMM supports uploading existing scripts or adding new scripts right in the dashboard. Languages supported are:
...
To download a Gueen RMM Script, click on the script in the Script Manager to select it. Then click the **Download Script** button on the top. You can also right-click on the script and select download

## Community Scripts

These are script that are built into Gueen RMM. They are provided and mantained by the Gueen RMM community. These scripts are updated whenever Gueen RMM is updated and can't be modified or deleted in the dashboard.
...
Gueen RMM allows scheduling tasks to run on agents. This leverages the Windows Task Scheduler and has the same scheduling options.
...
Gueen RMM allows passing in dashboard data to scripts as arguments. The below powershell arguments will get the client name of the agent and also the agent's public IP address
...
Gueen RMM supports pulling data from custom fields using the {{model.custom_field_name}} syntax.
...
Gueen RMM supports getting values from the global key store using the {{global.key_name}} syntax
