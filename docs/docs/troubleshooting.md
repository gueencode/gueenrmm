...
```bash
wget -N https://raw.githubusercontent.com/gueencode/gueenrmm/master/troubleshoot_server.sh
...
The IP address for all 3 should reflect your Gueen RMM server
...
Since Gueen RMM is still in alpha and the developers makes breaking changes pretty frequently, there is no promise of backwards compatibility.
...
First, reload NATS from gueen's web UI:<br>
...
```bash
/rmm/api/env/bin/python /rmm/api/gueenrmm/manage.py reload_nats
...
nats-server -DVV -c /rmm/api/gueenrmm/nats-rmm.conf
```
...
Run the gueenagent service manually with debug logging:

```cmd
"C:\Program Files\gueenAgent\gueenrmm.exe" -m winagentsvc -log debug -logto stdout
```

Run the gueenrpc service manually with debug logging:

```cmd
"C:\Program Files\gueenAgent\gueenrmm.exe" -m rpc -log debug -logto stdout
```

This will print out a ton of info. You should be able to see the error from the debug log output.

Please then copy/paste the logs and post them either in our [Discord support chat](https://discord.gg/upGTkWp), or create a [github issue](https://github.com/gueencode/gueenrmm/issues).

If all else fails, simply uninstall the agent either from control panel or silently with `"C:\Program Files\gueenAgent\unins000.exe" /VERYSILENT` and then reinstall the agent.
...
```bash
/rmm/api/gueenrmm/gueenrmm/private/log
/var/log/celery
```
...
Are you trying to use a proxy to share your single public IP with multiple services on 443? This is complicated and [unsupported by Gueen RMM](unsupported_scripts.md), test your setup.

## Mesh Agent x86 x64 integration with grmm
...
3. Find your gueenrmm group
4. Click the add link
5. Download both agents
6. In Gueen RMM, go **Settings > Global Settings > MeshCentral > Upload Mesh Agents** upload them both into the appropriate places.
