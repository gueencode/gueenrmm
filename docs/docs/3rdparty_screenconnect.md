...
## Install Gueen RMM via Screeconnect commands window
...
Invoke-WebRequest "<deployment URL>" -OutFile ( New-Item -Path "C:\temp\grmminstallx64.exe" -Force )
$proc = Start-Process c:\temp\grmminstallx64.exe -ArgumentList '-silent' -PassThru
Wait-Process -InputObject $proc

if ($proc.ExitCode -ne 0) {
    Write-Warning "$_ exited with status code $($proc.ExitCode)"
}
Remove-Item -Path "c:\temp\grmminstallx64.exe" -Force 
```
...
Invoke-WebRequest "<deployment URL>" -OutFile ( New-Item -Path "C:\temp\grmminstallx86.exe" -Force )
$proc = Start-Process c:\temp\grmminstallx86.exe -ArgumentList '-silent' -PassThru
Wait-Process -InputObject $proc

if ($proc.ExitCode -ne 0) {
    Write-Warning "$_ exited with status code $($proc.ExitCode)"
}
Remove-Item -Path "c:\temp\grmminstallx86.exe" -Force 
```
