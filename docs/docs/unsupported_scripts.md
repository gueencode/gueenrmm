...
!!!note
    These are not supported scripts/configurations by Gueen RMM, but it's provided here for your reference.
...
## General Notes on Proxies and Gueen RMM
...
Offsite Resource: <https://gitlab.com/NiceGuyIT/gueen-goodies/-/tree/main/traefik>
...
That it, you can now restart Gueen RMM and mesh.yourdomain.com should work, same for the agent.
...
The order of use_backend is important `gueen-Mesh-WebSocket_ipvANY` must be before `gueen-Mesh_ipvANY`
The values of `timeout connect`, `timeout server`, `timeout tunnel` in `gueen-Mesh-WebSocket` have been configured to maintain a stable agent connection, however you may need to adjust these values to suit your environment.
...
frontend HTTPS-merged
	...
	use_backend gueen_ipvANY  if  RMM aclcrt_RMM
	use_backend gueen_ipvANY  if  API aclcrt_API
	use_backend gueen-Mesh-WebSocket_ipvANY  if  is_websocket is_mesh aclcrt_MESH-WebSocket
	use_backend gueen-Mesh_ipvANY  if  MESH aclcrt_MESH
...
backend gueen_ipvANY
	...
	retries			3
	option			httpchk GET / 
	server			gueen 192.168.10.123:443 id 101 ssl check inter 1000  verify none 


backend gueen-Mesh-WebSocket_ipvANY
	mode			http
	id			113
	log			global
	...
	server			gueen 192.168.10.123:443 id 101 ssl  verify none 

backend gueen-Mesh_ipvANY
	mode			http
	id			112
	log			global
...
### Set gueen fail2ban filter conf File

```bash
gueenfail2banfilter="$(cat << EOF
[Definition]
failregex = ^<HOST>.*400.17.*$
ignoreregex = ^<HOST>.*200.*$
EOF
)"
sudo echo "${gueenfail2banfilter}" > /etc/fail2ban/filter.d/gueenrmm.conf
```

### Set gueen fail2ban jail conf File

```bash
gueenfail2banjail="$(cat << EOF
[gueenrmm]
enabled = true
port = 80,443
filter = gueenrmm
action = iptables-allports[name=gueen]
logpath = /rmm/api/gueenrmm/gueenrmm/private/log/access.log
maxretry = 3
bantime = 14400
findtime = 14400
EOF
)"
sudo echo "${gueenfail2banjail}" > /etc/fail2ban/jail.d/gueenrmm.local
```
...
How to change certs used by Gueen RMM to purchased ones (this can be a wildcard cert).
...
`/rmm/api/gueenrmm/gueenrmm/local_settings.py`

1. create a new folder for certs and allow gueen user permissions (assumed to be gueen)

		sudo mkdir /certs
		sudo chown -R gueen:gueen /certs"
...
5. Add the following to the last lines of `/rmm/api/gueenrmm/gueenrmm/local_settings.py`

        nano /rmm/api/gueenrmm/gueenrmm/local_settings.py
...
The standard SSL cert process in gueen uses a [DNS challenge](https://letsencrypt.org/docs/challenge-types/#dns-01-challenge) that requires dns txt files to be updated in your public DNS with every cert renewal.

The below script uses [http challenge](https://letsencrypt.org/docs/challenge-types/#http-01-challenge) on the 3 separate ssl certs, one for each subdomain: rmm, api, mesh. They still have the same 3 month expiry. Restart the Gueen RMM server about every 2.5 months (80 days) for auto-renewed certs to become active.

!!!note
    Your Gueen RMM server will need to have TCP Port: 80 exposed to the internet

```bash
#!/bin/bash

###Set colours same as Gueen RMM install and Update
YELLOW='\033[1;33m'
GREEN='\033[0;32m'
RED='\033[0;31m'
NC='\033[0m'
...
### Setup gueen nginx config files for letsencrypt
...
echo "${rmmlocalsettings}" | tee --append /rmm/api/gueenrmm/gueenrmm/local_settings.py > /dev/null
...
Limit access to Gueen RMM's administration panel in nginx to specific locations
