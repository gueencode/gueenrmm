...
Add the following to the top of the file right under the `upstream gueenrmm {...}` block
```bash
...
Look for this line
```bash
access_log /rmm/api/gueenrmm/gueenrmm/private/log/access.log;
```
and change to
```bash
...
5. Replace `/rmm/api/gueenrmm/app.ini` with the following:
```bash
[uwsgi]
...
8. Clear log files
```bash
baselog="/rmm/api/gueenrmm/gueenrmm/private/log"
...
```bash
wget https://raw.githubusercontent.com/gueencode/gueenrmm/master/update.sh
chmod +x update.sh
./update.sh
```
