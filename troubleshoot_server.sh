...
# Gueen RMM install troubleshooting script
...
if [ "$locapiip" = "$remapiip" ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED} Your Local and Remote IP for $rmmdomain all agents will require non-public DNS to find grmm server | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ "$locrmmip" = "$remrmmip" ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED} echo Your Local and Remote IP for $frontenddomain all agents will require non-public DNS to find grmm server | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ "$locmeship" = "$remmeship" ]; then
    ...
    printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED} Your Local and Remote IP for $meshdomain all agents will require non-public DNS to find grmm server | tee -a checklog.log
	printf >&2 "\n\n" | tee -a checklog.log
...
if [ $rmmstatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'RMM Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ $daphnestatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'daphne Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ $celerystatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'celery Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ $celerybeatstatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'celerybeat Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ $nginxstatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'nginx Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ $natsstatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'nats Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
...
if [ $natsapistatus = active ]; then
	...
	printf >&2 "\n\n" | tee -a checklog.log
    echo -ne ${RED}  'nats-api Service isnt running (gueen wont work without this)' | tee -a checklog.log
	printf >&2 "\n\n"
