      ...
      <q-form @submit="sendRecovery">
        <q-card-section>
          <div class="q-gutter-sm">
            ...
            <q-radio dense v-model="state.mode" val="rpc" label="gueen RPC" />
            <q-radio dense v-model="state.mode" val="tacagent" label="gueen Agent" />
        ...
        <q-card-section v-else-if="state.mode === 'tacagent'">
          Fix issues with the gueenAgent windows service which handles agent check-in.
        </q-card-section>
        <q-card-section v-else-if="state.mode === 'rpc'">
          Fix issues with the gueen RPC service which handles most of the agent's realtime functions and scheduled
          tasks.
        </q-card-section>
        <q-card-section v-else-if="state.mode === 'command'">
          ...
          <p>Only use this as a last resort if unable to recover the gueen RPC service.</p>
          <q-input
            ref="input"
            v-model="state.cmd"
            outlined
