...
from gueenrmm.test import gueenTestCase
...
class TestAgentsList(gueenTestCase):
    def setUp(self):
        self.authenticate()
...
class TestAgentViews(gueenTestCase):
    def setUp(self):
        self.authenticate()
        self.setup_coresettings()
    ...
    def test_get_processes(self, mock_ret):
        ...
        with open(
            os.path.join(settings.BASE_DIR, "gueenrmm/test_data/procs.json")
        ) as f:
    ...
    def test_get_event_log(self, nats_cmd):
        ...
        with open(
            os.path.join(settings.BASE_DIR, "gueenrmm/test_data/appeventlog.json")
        ) as f:
...
class TestAgentViewsNew(gueenTestCase):
    def setUp(self):
        self.authenticate()
        self.setup_coresettings()
...
class TestAgentPermissions(gueenTestCase):
    def setUp(self):
        self.client_setup()
        self.setup_coresettings()
...
class TestAgentTasks(gueenTestCase):
    def setUp(self):
        self.authenticate()
        self.setup_coresettings()
    ...
    def test_agent_update(self, nats_cmd, get_url):
        get_url.return_value = "https://exe.gueenrmm.io"
        ...
        self.assertEqual(
            action.details["url"],
            f"https://github.com/gueencode/rmmagent/releases/download/v{settings.LATEST_AGENT_VER}/winagent-v{settings.LATEST_AGENT_VER}.exe",
        )
        self.assertEqual(
        ...
        nats_cmd.assert_called_with(
            {
                "func": "agentupdate",
                "payload": {
                    "url": f"https://github.com/gueencode/rmmagent/releases/download/v{settings.LATEST_AGENT_VER}/winagent-v{settings.LATEST_AGENT_VER}.exe",
                    "version": settings.LATEST_AGENT_VER,
        ...
        nats_cmd.return_value = "ok"
        get_exe.return_value = "https://exe.gueenrmm.io"
        ...
        nats_cmd.assert_called_with(
            {
                "func": "agentupdate",
                "payload": {
                    "url": f"https://exe.gueenrmm.io/api/v1/winagents/?version={settings.LATEST_AGENT_VER}&arch=64&token=testtoken123",  # type: ignore
                    "version": settings.LATEST_AGENT_VER,
                    "inno": f"winagent-v{settings.LATEST_AGENT_VER}.exe",
                },
            },
        ...
        nats_cmd.assert_called_with(
            {
                "func": "agentupdate",
                "payload": {
                    "url": f"https://exe.gueenrmm.io/api/v1/winagents/?version={settings.LATEST_AGENT_VER}&arch=32&token=testtoken123",  # type: ignore
                    "version": settings.LATEST_AGENT_VER,
