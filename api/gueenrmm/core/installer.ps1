...
$serviceName = 'gueenagent'
If (Get-Service $serviceName -ErrorAction SilentlyContinue) {
    write-host ('Gueen RMM Is Already Installed')
} Else {
    $OutPath = $env:TMP
    $output = $innosetup
    ...
    if ($connectresult.TcpTestSucceeded -eq $true){
        ...
        {  
            ...
            Start-Sleep -s 5
            Start-Process -FilePath "C:\Program Files\gueenAgent\gueenrmm.exe" -ArgumentList $installArgs -Wait
            exit 0
        }
        Catch
        {
