...
class TestAutotaskViews(gueenTestCase):
    def setUp(self):
        self.authenticate()
...
class TestAutoTaskCeleryTasks(gueenTestCase):
    def setUp(self):
        self.authenticate()
        self.setup_coresettings()
    ...
    def test_remove_orphaned_win_task(self, nats_cmd):
        ...
        win_tasks = [
            ...
            "gueenrmm_fixmesh",
            "gueenrmm_SchedReboot_jk324kajd",
            "gueenrmm_iggrLcOaldIZnUzLuJWPLNwikiOoJJHHznb",  # orphaned task
        ]
            ...
            call(
                {
                    "func": "delschedtask",
                    "schedtaskpayload": {
                        "name": "gueenrmm_iggrLcOaldIZnUzLuJWPLNwikiOoJJHHznb"
                    },
                },
                timeout=10,
            ),
        ...
        # no orphaned tasks
        nats_cmd.reset_mock()
        win_tasks.remove("gueenrmm_iggrLcOaldIZnUzLuJWPLNwikiOoJJHHznb")
        nats_cmd.side_effect = [win_tasks, "ok"]
        ret = remove_orphaned_win_tasks.s(self.agent.pk).apply()
...
class TestTaskPermissions(gueenTestCase):
    def setUp(self):
        self.setup_coresettings()
        self.client_setup()
